---
title: "Guía 10 Análisis de varianza y posteriores"
...
Utilice el siguiente ejemplo para completar la información de metadatos: Título: Reporte Anova y posteriores, nombre del autor: Su nombre.
...
**c).** Realice un análisis exploratorio de datos de la variable bajo estudio. A partir del set de datos de elabore un gráfico de cajas y bigotes para la variable **Final_Weight_g**. 
...
Tilapia_nilotica %>%
          ggplot(aes(x= Cepa,y=Final_Weight_g,fill=Cepa))+
                ...
                labs(x="Cepa",y="Final_Weight (g)",title="Distribución entre las variables Final_Weight vs Cepa")

```

**d).** Incluya una tabla con los estimadores puntuales de los promedios y las varianzas de la variable Final_Weight_g para cada nivel del factor Cepa.

```{r, echo=TRUE, warning=FALSE,message=FALSE}
# Promedio y varianza de la variable Final_Weight_g para la Cepa Spring. 

M_V_Final_Weight_g_Spring <- Tilapia_nilotica %>%
                             filter(Cepa == "Spring") %>%
summarize(media_Final_Weight_g_Spring= mean(Final_Weight_g),
          varianza_Final_Weight_g_Spring= var(Final_Weight_g))

M_V_Final_Weight_g_Spring

# Promedio y varianza de la variable Final_Weight_g para la Cepa B. 
M_V_Final_Weight_g_B <- Tilapia_nilotica %>%
                        filter(Cepa == "B") %>%
summarize(media_Final_Weight_g_B= mean(Final_Weight_g),
          varianza_Final_Weight_g_B= var(Final_Weight_g))

M_V_Final_Weight_g_B

Tabla <- data.frame(M_V_Final_Weight_g_Spring, M_V_Final_Weight_g_B)
colnames(Tabla) <- c("Peso_Final_Promedio_Cepa_Spring","Varianza_Peso_Final_Spring", "Peso_Final_Promedio_Cepa_B","Varianza_Peso_Final_B")

knitr::kable(Tabla,caption ="Medias y varianzas de la variable Final_Weight(g) para las cepas Spring y B")
```


**e).** Realice el ajuste del siguiente modelo usando la función aov(), donde los parámetros son los siguientes: variable respuesta **Final_Weight_g** y como factor de clasificación **Cepa**, como set de datos (data=Tilapia_nilotica). Llame al objeto model1_anova1. Utilice el simbolo ~ para relacionar la variable respuesta con el factor de clasificación. Luego, realice el anova() del objeto model1_anova1. Use la función pander::pander() para mostrar los resultados del modelo (model1_anova1), coloque en el parámetro caption "ANOVA a una vía de clasificación usando la función aov()."

```{r}
model1_anova1 <- aov(Final_Weight_g ~ Cepa, data=Tilapia_nilotica)
anova(model1_anova1)
pander::pander(model1_anova1, caption = "ANOVA a una vía de clasificación usando la función aov().")
```
...
**c).** Realice un análisis exploratorio de datos de la variable bajo estudio. A partir del set de datos de elabore un gráfico de cajas y bigotes para la variable indice_condicion_corporal.
...
Trucha %>%
          ggplot(aes(x=Tiempo_Dias,y=indice_condicion_corporal,fill=Tiempo_Dias))+
                geom_boxplot()+
                theme(legend.position="none")+
                labs(x="Tiempo_Dias",y="Indice_condicion_corporal",title="Distribución entre las variables Indice_condicion_corporal vs Tiempo_Dias")
```

**d).** Calcule los estimadores puntuales de los promedios y las varianzas de la variable indice_condicion_corporal para cada nivel del factor Tiempo_Dias.

```{r}
M_V_ICC_15 <- Trucha %>%
              filter(Tiempo_Dias == "15") %>%
summarize(media_ICC_15= mean(indice_condicion_corporal),
          varianza_ICC_15= var(indice_condicion_corporal))
...
M_V_ICC_30 <- Trucha %>%
              filter(Tiempo_Dias == "30") %>%
summarize(media_ICC_30= mean(indice_condicion_corporal),
          varianza_ICC_30= var(indice_condicion_corporal))

M_V_ICC_30


M_V_ICC_45 <- Trucha %>%
              filter(Tiempo_Dias == "45") %>%
summarize(media_ICC_45= mean(indice_condicion_corporal),
          varianza_ICC_45= var(indice_condicion_corporal))

M_V_ICC_45

```

**e).** Realice el ajuste del siguiente modelo usando la función aov(), donde los parámetros son los siguientes: variable respuesta indice_condicion_corporal y como factor de clasificación Tiempo_Dias, como set de datos (data=Trucha). Llame al objeto model2_anova1. Utilice el simbolo ~ para relacionar la variable respuesta con el factor de clasificación. Realice el anova() del objeto model2_anova1. Use la función pander::pander() para mostrar los resultados del modelo (model2_anova1), coloque en el parámetro caption un título a la tabla.

```{r}
model2_anova1 <- aov(indice_condicion_corporal ~ Tiempo_Dias, data=Trucha)
anova(model2_anova1)
pander::pander(model2_anova1, caption = "ANOVA a un criterio de clasificación con tres niveles usando la función aov().")
```
...
**d).** Halle los estimadores puntuales de los promedios y las varianzas de la variable Initial_Weight para cada nivel del factor Diet y para cada nivel del factor Tank.

```{r}
# Promedios y varianzas de la variable Initial_Weight para cada dieta "Diet" (Animal y Vegetal) y en cada Tank (1, 2 y 3)

M_V_IW_Animal_T1 <- Camarones %>%
                    filter(Diet == "Animal", Tank == "1") %>%
                  summarize(media_IW_A_T1=mean(Initial_Weight),
                          varianza_IW_A_T1= var(Initial_Weight))
M_V_IW_Animal_T1


M_V_IW_Animal_T2 <- Camarones %>%
                    filter(Diet == "Animal", Tank == "2") %>%
                  summarize(media_IW_A_T2=mean(Initial_Weight),
                          varianza_IW_A_T2= var(Initial_Weight))
M_V_IW_Animal_T2


M_V_IW_Animal_T3 <- Camarones %>%
                    filter(Diet == "Animal", Tank == "3") %>%
                  summarize(media_IW_A_T3=mean(Initial_Weight),
                          varianza_IW_A_T3= var(Initial_Weight))
M_V_IW_Animal_T3


M_V_IW_Vegetal_T1 <- Camarones %>%
                    filter(Diet == "Vegetal", Tank == "1") %>%
                  summarize(media_IW_V_T1=mean(Initial_Weight),
                          varianza_IW_V_T1= var(Initial_Weight))
M_V_IW_Vegetal_T1


M_V_IW_Vegetal_T2 <- Camarones %>%
                    filter(Diet == "Vegetal", Tank == "2") %>%
                  summarize(media_IW_V_T2=mean(Initial_Weight),
                          varianza_IW_V_T2= var(Initial_Weight))
M_V_IW_Vegetal_T2


M_V_IW_Vegetal_T3 <- Camarones %>%
                    filter(Diet == "Vegetal", Tank == "3") %>%
                  summarize(media_IW_V_T3=mean(Initial_Weight),
                          varianza_IW_V_T3= var(Initial_Weight))
M_V_IW_Vegetal_T3
