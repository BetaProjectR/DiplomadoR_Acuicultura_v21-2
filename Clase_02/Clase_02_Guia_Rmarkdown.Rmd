...
En el primer bloque de códigos o **chunk** asegúrese de que los comandos están configurados de la siguiente manera *knitr::opts_chunk$set(echo = TRUE)*.Luego cargue la librería **stats** usando la función library().
...
Incluya y describa en su introducción la siguiente tabla de datos usando markdown. Note que **Time** está justificado a la izquierda y **demand** a la derecha.
...
Las tablas también se pueden hacer dentro de un bloque de códigos. Incluya en su reporte la misma tabla anterior pero esta vez usando este comando **knitr::kable()**. Use el argumento *caption =""* para agregar la leyenda de la tabla.
...
Finalmente inprima diferentes versiones de su reporte (pdf, word, html) con el botón **Knit**.
